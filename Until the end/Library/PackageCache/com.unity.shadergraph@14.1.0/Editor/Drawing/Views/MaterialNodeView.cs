    ...
    sealed class MaterialNodeView : Node, IShaderNodeView, IInspectable
    {
        ...
        public void Initialize(AbstractMaterialNode inNode, PreviewManager previewManager, IEdgeConnectorListener connectorListener, MaterialGraphView graphView)
        {
            ...
            switch (node)
            {
                ...
                case GetLocalVariableNode:
                    RegisterCallback<MouseDownEvent>(OnGetLocalVariableDoubleClick);
                    m_UnregisterAll += () => { UnregisterCallback<MouseDownEvent>(OnGetLocalVariableDoubleClick); };
                    break;
        ...
        void OnGetLocalVariableDoubleClick(MouseDownEvent evt)
        {
            if (evt.clickCount == 2 && evt.button == 0 && node.owner.localVariables.Any())
            {
                GetLocalVariableNode getLocalVariableNode = node as GetLocalVariableNode;

                LocalVariableRegisterNode localVariableRegisterNode = getLocalVariableNode.owner.GetLocalVariableRegisterNode(getLocalVariableNode.variableName);
                var nodeList = m_GraphView.Query<MaterialNodeView>().ToList();
                nodeList.ForEach(nodeView =>
                {
                    if (nodeView.node.objectId == localVariableRegisterNode.objectId)
                    {
                        m_GraphView.RemoveFromSelection(gvNode);
        ...
        public void UpdateNodeLOD(int lodLevel)
        {
            node.nodeLOD = lodLevel;
        }
        ...
        IEnumerable<MaterialNodeView> GetAssociatedSelectionLocalVarNode()
        {
            if (node is LocalVariableRegisterNode && !((LocalVariableRegisterNode)node).conflictRegister)
            {
                var variable = (node as LocalVariableRegisterNode).variableName;
                var getVarNodes = m_GraphView.Query<MaterialNodeView>().ToList().Where(v => v.node is GetLocalVariableNode);
                return getVarNodes.Where(v => ((GetLocalVariableNode)v.node).variableName.Equals(variable));
            }
            if (node is GetLocalVariableNode)
            {
                var variable = (node as GetLocalVariableNode).variableName;
                var registerNodes = m_GraphView.Query<MaterialNodeView>().ToList().Where(v => v.node is LocalVariableRegisterNode);
                return registerNodes.Where(v => ((LocalVariableRegisterNode)v.node).variableName.Equals(variable) && !((LocalVariableRegisterNode)v.node).conflictRegister);
            }
            return null;
        }
        IEnumerable<MaterialNodeView> GetConnectedLoopNodes()
        {
            if (node is LoopNode)
            {
                var index = (node as LoopNode).loopIndex;
                bool isStartNode = node is LoopStart;
                var otherSide = isStartNode ? m_GraphView.graph.GetLoopEndNode(index) : m_GraphView.graph.GetLoopStartNode(index);
                if (otherSide != null)
                {
                    List<AbstractMaterialNode> outputNodes = new List<AbstractMaterialNode>();
                    NodeUtils.CollectNodesNodeFeedsInto(outputNodes, isStartNode ? node : otherSide, NodeUtils.IncludeSelf.Exclude);
                    List<AbstractMaterialNode> inputNodes = new List<AbstractMaterialNode>();
                    NodeUtils.DepthFirstCollectNodesFromNode(inputNodes, isStartNode ? otherSide : node, NodeUtils.IncludeSelf.Exclude);
                    var connectedNodes = outputNodes.Intersect(inputNodes);
                    return m_GraphView.Query<MaterialNodeView>().ToList().Where(v => (connectedNodes.Contains(v.node) || (v.node == otherSide))); 
                }
            }
            return null;
        }
        void OnMouseHover(EventBase evt)
        {
            ...
            var localVarNodeViews = GetAssociatedSelectionLocalVarNode();
            if (localVarNodeViews != null)
            {
                foreach (var nodeView in localVarNodeViews)
                {
                    if (evt.eventTypeId == MouseEnterEvent.TypeId())
                    {
                        nodeView.AddToClassList("hovered");
                    }
                    else
                    {
                        nodeView.RemoveFromClassList("hovered");
                    }
                }
            }
            var connectedLoopNodes = GetConnectedLoopNodes();
            
            if (connectedLoopNodes != null)
            {
                foreach (var nodeView in connectedLoopNodes)
                {
                    if (evt.eventTypeId == MouseEnterEvent.TypeId())
                    {
                        nodeView.AddToClassList("hovered");
        ...
        {
            if (m_PreviewRenderData.texture == null || !node.previewExpanded || node.nodeLOD == 2) 
            {
