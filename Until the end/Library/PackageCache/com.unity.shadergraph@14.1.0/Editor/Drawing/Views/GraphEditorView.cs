...
namespace UnityEditor.ShaderGraph.Drawing
{
    ...
    {
        public bool isStatisticsVisible = false;
    ...
    class GraphEditorView : VisualElement, IDisposable
    {
        ...
        public GraphEditorView(EditorWindow editorWindow, GraphData graph, MessageManager messageManager, string graphName)
        {
            ...
            var toolbar = new IMGUIContainer(() =>
            {
                ...
                if (GUILayout.Button("Save Asset", EditorStyles.toolbarButton))
                {
                    ...
                    {
                        saveRequested();
                        previewManager.UpdateMasterPreview(ModificationScope.Graph);
                    }
                }
                GUILayout.Space(6);
                ...
                GUILayout.Space(4);
                m_UserViewSettings.isStatisticsVisible = GUILayout.Toggle(m_UserViewSettings.isStatisticsVisible, "Stats", EditorStyles.toolbarButton);
            ...
            {
                // We can think during the process of Adding Nodes, the Shader Group will not be changed.
            ...
            m_GraphView.RegisterCallback<WheelEvent>(NodeLOD);
        ...
        void UpdateSubWindowsVisibility()
        {
            ...
            m_StatisticsView.visible = m_UserViewSettings.isStatisticsVisible;
        ...
        {
            m_StatisticsView = new StatisticsView() { name = "shaderStatistics" };
            m_GraphView.Add(m_StatisticsView);
            previewManager.UpdateMasterPreview(ModificationScope.Graph);
        }
        ...
        private void UpdateNodeLODs()
        {
            int lodLevel = 0;
            if (m_GraphView.scale < 0.25f)
                lodLevel = 2;

            var nodeList = m_GraphView.Query<MaterialNodeView>().ToList();
            foreach (var node in nodeList)
            {
                if (node.node.nodeLOD != lodLevel)
                    node.UpdateNodeLOD(lodLevel);
            }
        }

        private void NodeLOD(WheelEvent evt)
        {
            UpdateNodeLODs();
        }
        ...
        float lastUpdate = 0f;
        public void HandleGraphChanges(bool wasUndoRedoPerformed, bool isFinalization = false)
        {
            ...
            m_GraphElementsTemp.AddRange(m_GraphView.graphElements.ToList().OfType<ShaderGroup>());
            ...
            if (m_Graph.mostRecentlyCreatedGroup != null)
            {
                ...
                {
                    if (shaderGroup.userData == m_Graph.mostRecentlyCreatedGroup && !shaderGroup.userData.loopGroup)
                    {
            ...
            //if (m_Graph.pastedGroups.Count() > 0 || m_Graph.addedGroups.Count() > 0 || wasUndoRedoPerformed || m_InspectorView.doesInspectorNeedUpdate)
            {
                var shaderGroups = m_GraphView.graphElements.OfType<ShaderGroup>().ToList();
                foreach (ShaderGroup shaderGroup in shaderGroups)
                {
                    if (shaderGroup.userData.loopGroup)
                    {
                        shaderGroup.style.backgroundColor = new Color(1, 0.5f, 0.0f, 0.1f);
                        Color borderColor = new Color(1, 0.5f, 0, 1);
                        shaderGroup.style.borderBottomColor = borderColor;
                        shaderGroup.style.borderTopColor = borderColor;
                        shaderGroup.style.borderLeftColor = borderColor;
                        shaderGroup.style.borderRightColor = borderColor;
                    }
                }
            }
        ...
        List<ShaderGroup> m_GraphElementsTemp = new List<ShaderGroup>();
        ...
        void AddNode(AbstractMaterialNode node, bool usePrebuiltVisualGroupMap = false, Dictionary<object, GraphElement> lookupTable = null)
        {
            ...
            if (materialNode.group != null)
            {
                ...
                else
                {
                    ...
                    foreach (var element in m_GraphElementsTemp)
                    {
                        if (element.userData == materialNode.group)
                        {
                            element.AddElement(nodeView);
                            // One element can just be put into one Shader Group.
                            break;
                        }
                    }
                }
        ...
        void AddStickyNote(StickyNoteData stickyNoteData, Dictionary<object, GraphElement> lookupTable = null)
        {
            ...
            foreach (var element in m_GraphElementsTemp)
            {
                if (element.userData == stickyNoteData.group)
                {
                    element.AddElement(stickyNote);
                    // One element can just be put into one Shader Group.
                    break;
                }
            }
        ...
        bool IsTextureSampleNode(AbstractMaterialNode node)
        {
            return node is SampleTexture2DNode ||
                node is SampleTexture2DArrayNode ||
                node is SampleTexture3DNode ||
                node is SampleCubemapNode ||
                node is SampleRawCubemapNode ||
                node is SampleTexture2DLODNode;
        }

        public void UpdateStats(ref GraphData graph)
        {
            string statsString = string.Empty;
            var vertStats = ShaderUtil.GetCompiledInfo(previewManager.masterRenderData.shaderData.mat, ShaderType.Vertex);
            var fragStats = ShaderUtil.GetCompiledInfo(previewManager.masterRenderData.shaderData.mat, ShaderType.Fragment);

            if (vertStats.Success)
            {
                var blockNodes = m_Graph.GetNodes<BlockNode>().ToList();
                using (var propagateNodes = PooledHashSet<AbstractMaterialNode>.Get())
                {
                    blockNodes.ForEach(bn => propagateNodes.Add(bn));
                    PreviewManager.PropagateNodes(propagateNodes, PreviewManager.PropagationDirection.Upstream, propagateNodes);
                    var texSampleCount = propagateNodes.Where(n => IsTextureSampleNode(n)).Count();
                    propagateNodes.Where(n => n is ParallaxOcclusionMappingNode).ToList().ForEach(n => texSampleCount += (int)n.FindInputSlot<Vector1MaterialSlot>(5).value);

                    statsString = string.Empty;

                    // Vertex Stats
                    statsString += fragStats.Success ? "Vertex Shader Stats: \n   - " : "";     // UnityShaderCompiler will merge the compilation results into vertex shader program under OpenGL platforms (eg. Ubuntu)
                    statsString += $"ConstantBuffer Count: {vertStats.ConstantBufferCount} \n   - ";
                    statsString += $"NumSamplers: {vertStats.SamplerBits.Length} \n   - ";
                    statsString += $"Resources Count: {vertStats.TextureBindings.Length} \n   - ";
                    statsString += "Attributes Used: ";
                    for (int i = 0; i < vertStats.Attributes.Length; i++)
                        statsString += i == vertStats.Attributes.Length - 1 ? vertStats.Attributes[i] + " \n\n" : vertStats.Attributes[i] + ", ";

                    if (fragStats.Success)
                    {
                        // Fragment Stats
                        statsString += "Fragment Shader Stats: \n   - ";
                        statsString += $"ConstantBuffer Count: {fragStats.ConstantBufferCount} \n   - ";
                        statsString += $"NumSamplers: {fragStats.SamplerBits.Length} \n   - ";
                        statsString += $"Resources Count: {fragStats.TextureBindings.Length} \n\n";
                    }

                    // Common Stats
                    statsString += $"Total Samplers: {vertStats.SamplerBits.Union(fragStats.SamplerBits).Count()}/16 \n";
                    statsString += $"Texture Sample Count: {texSampleCount} \n";
                    statsString += $"Custom Interpolator Count: {blockNodes.Where(bn => bn.isCustomBlock == true).Count()}";
                }
            }
            else
            {
                if (ShaderUtil.ShaderHasError(previewManager.masterRenderData.shaderData.shader))
                    statsString = "Shader has error...";

                if (graph.masterNodeShaderStats == string.Empty)
                    statsString = "Compiling Shader...";
            }

            if (statsString != string.Empty)
                (m_StatisticsView.Children().FirstOrDefault() as Label).text = graph.masterNodeShaderStats = statsString;
        }
